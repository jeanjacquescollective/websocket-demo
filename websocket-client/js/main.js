...
const connectionStateInput = document.querySelector("#connection-state");
...
connectButton.addEventListener("click", () => {
  ...
  socket.addEventListener("open", () => {
    ...
    connectionStateInput.innerText = "Connected";
  ...
  socket.addEventListener("close", () => {
    ...
    connectionStateInput.innerText = "Disconnected";
...
slider1.addEventListener("input", () => {
  ...
  if (socket && socket.readyState === WebSocket.OPEN) {
    socket.send(JSON.stringify({ "slider1": slider1.value }));
  }
});
